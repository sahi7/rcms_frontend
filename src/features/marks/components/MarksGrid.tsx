...
export default function MarksGrid({
    ...
    // Extract class number: "Form 1", "Form 5", "Senior 2" → 1, 5, 2
    const classMatch = batch.class_name.match(/\d+/);
    const classNumber = classMatch ? parseInt(classMatch[0], 10) : 1;
    const isSenior = classNumber >= 5;
    ...
    const handleScoreChange = (index: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const getGrade = (percentage: number): string => {
            ...
            if (isSenior) {
                // Senior: Pass = E and above (40%+)
                if (percentage >= 60) return "C";
                if (percentage >= 50) return "D";
                if (percentage >= 40) return "E";
                return "F"; // Fail
            } else {
                // Junior: Pass = C and above (50%+)
                if (percentage >= 50) return "C";
                if (percentage >= 40) return "D";
                if (percentage >= 30) return "F";
                return "U"; // Fail (or "U" if you want)
            }
        };
        ...
        setMarks(prev =>
            prev.map((m, i) => {
                ...
                const grade = score === null ? "" : getGrade(percentage);
                                                ...
                                                <Badge
                                                    variant={
                                                        mark.grade === "A" || mark.grade === "B"
                                                            ? "default"
                                                            : (isSenior && ["C", "D", "E"].includes(mark.grade || "")) ||
                                                                (!isSenior && ["C"].includes(mark.grade || ""))
                                                                ? "secondary"
                                                                : mark.grade === "F"
                                                                    ? "destructive"
                                                                    : "secondary"
