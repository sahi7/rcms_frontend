...
export default function DownloadsPage() {
    ...
    const handleDownload = async (url: string, filename: string) => {
        ...
        // If we already know it's missing, don't try
        ...
        try {
            ...
            // File exists → trigger real download
            ...
            // File missing or error → mark as failed
            setFailedUrls(prev => new Set(prev).add(url));
            // Optional: toast.error("File not found on server");
        } finally {
                        ...
                        {filteredJobs.map((job) => (
                            <TableRow key={job.id}>
                                ...
                                <TableCell className="text-right">
                                    {failedUrls.has(job.zip_file_url) ? (
                            ...
                            {failedUrls.has(job.zip_file_url) ? (
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs text-red-600 text-center">
                                        This report was deleted from the server
                                    </p>
                                </div>
                            ) : (
                                <Button
